...
output "name_id" {
  ...
  value = {
    for key, resource in panos_device_group.this : resource.name => base64encode(jsonencode({
      name = resource.name
...
output "devices" {
  description = "Resource name to assigned device serials, read from provider state."
  value = { for key, resource in panos_device_group.this : resource.name => [
    for device in coalesce(resource.devices, []) : device.name
  ] }
}
