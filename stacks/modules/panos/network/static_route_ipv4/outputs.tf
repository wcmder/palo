...
output "name_id" {
  ...
  value = {
    for key, resource in panos_virtual_router_static_route_ipv4.this : resource.name => base64encode(jsonencode({
      name           = resource.name
      virtual_router = resource.virtual_router
...
output "names" {
  ...
  value       = { for key, resource in panos_virtual_router_static_route_ipv4.this : key => resource.name }
}
