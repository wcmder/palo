...
output "name_id" {
  ...
  value = {
    for key, resource in panos_service_group.this : resource.name => base64encode(jsonencode({
      name = resource.name
...
output "names" {
  ...
  value       = { for key, resource in panos_service_group.this : key => resource.name }
}
