...
output "name_id" {
  ...
  value = {
    for key, resource in panos_address_group.this : resource.name => base64encode(jsonencode({
      name = resource.name
...
output "names" {
  ...
  value       = { for key, resource in panos_address_group.this : key => resource.name }
}
