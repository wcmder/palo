...
output "name_id" {
  ...
  value = {
    for key, resource in panos_administrative_tag.this : resource.name => base64encode(jsonencode({
      name = resource.name
...
output "names" {
  ...
  value       = { for key, resource in panos_administrative_tag.this : key => resource.name }
}
