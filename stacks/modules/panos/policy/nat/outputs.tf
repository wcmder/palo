...
output "name_id" {
  ...
  value = {
    for key, resource in panos_nat_policy.this : key => base64encode(jsonencode({
...
output "names" {
  ...
  value       = { for key, resource in panos_nat_policy.this : key => [for rule in resource.rules : rule.name] }
}
